...
def download_libre_hardware_monitor_dll():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if dll_found:
                print(f"✅ LibreHardwareMonitorLib.dll downloaded successfully to: {dll_path}")
                return True
            else:
                print("❌ LibreHardwareMonitorLib.dll not found in the downloaded package")
                return False
                
    except Exception as e:
        print(f"❌ Failed to download LibreHardwareMonitorLib.dll: {e}")
        print("\nManual download instructions:")
        print("1. Go to: https://github.com/LibreHardwareMonitor/LibreHardwareMonitor/releases/latest")
